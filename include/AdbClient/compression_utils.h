...
struct Decoder {
    ...
    bool Finish() {
        ...
        if (old) {
            error = "Decoder::Finish called while already finished?";
            return false;
        }
        return true;
    }
...
struct NullDecoder final : public Decoder {
    ...
    DecodeResult Decode(std::span<char>* output) final {
        ...
        while (available_out > 0 && !input_buffer_.empty()) {
            size_t len = (available_out < input_buffer_.front_size() ? available_out : input_buffer_.front_size());
...
struct LZ4Decoder final : public Decoder {
    explicit LZ4Decoder(std::span<char> output_buffer)
        ...
        if (LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION) != 0) {
            error = "failed to initialize LZ4 decompression context";
        }
    ...
    DecodeResult Decode(std::span<char>* output) final {
        ...
        if (LZ4F_isError(rc)) {
            error = std::string("LZ4F_decompress failed: ") + LZ4F_getErrorName(rc);
            return DecodeResult::Error;
        }
        ...
        if (rc == 0) {
            if (!input_buffer_.empty()) {
                error = "LZ4 stream hit end before reading all data";
                return DecodeResult::Error;
            }
            lz4_done_ = true;
        }
...
struct LZ4Encoder final : public Encoder {
    explicit LZ4Encoder(size_t output_block_size)
        ...
        if (LZ4F_createCompressionContext(&cctx, LZ4F_VERSION) != 0) {
            error = "failed to initialize LZ4 compression context";
        }
        ...
        if (LZ4F_isError(rc)) {
            error = StringPrintf("LZ4F_compressBegin failed: %s", LZ4F_getErrorName(rc));
        }
        header.resize(rc);
    ...
    EncodeResult Encode(Block* output) final {
        ...
        if (available_in != 0) {
            if (lz4_finalized_) {
                error = "LZ4Encoder received data after Finish?";
                return EncodeResult::Error;
            }

            available_in = (available_in < max_input_size ? available_in : max_input_size);
            ...
            size_t rc = LZ4F_compressUpdate(encoder_.get(), next_out, available_out, next_in,
                available_in, nullptr);
            if (LZ4F_isError(rc)) {
                error = std::string("LZ4F_compressUpdate failed: ") + LZ4F_getErrorName(rc);
                return EncodeResult::Error;
            }
        ...
        if (finished_ && !lz4_finalized_) {
            ...
            size_t rc = LZ4F_compressEnd(encoder_.get(), final_block.data(), final_block.size(),
                nullptr);
            if (LZ4F_isError(rc)) {
                error = std::string("LZ4F_compressEnd failed: ") + LZ4F_getErrorName(rc);
                return EncodeResult::Error;
            }
        ...
        if (OutputReady()) {
            size_t len = (output_block_size_< output_buffer_.size()? output_block_size_ : output_buffer_.size());
...
struct ZstdDecoder final : public Decoder {
    explicit ZstdDecoder(std::span<char> output_buffer)
        ...
        if (!decoder_) {
            error = "failed to initialize Zstd decompression context";
        }
    }
    ...
    DecodeResult Decode(std::span<char>* output) final {
        ...
        if (ZSTD_isError(rc)) {
            error = std::string("ZSTD_decompressStream failed: ") + ZSTD_getErrorName(rc);
            return DecodeResult::Error;
        }
        ...
        if (rc == 0) {
            if (!input_buffer_.empty()) {
                error = "Zstd stream hit end before reading all data";
                return DecodeResult::Error;
            }
            zstd_done_ = true;
        }
...
struct ZstdEncoder final : public Encoder {
    ...
    EncodeResult Encode(Block* output) final {
        ...
        if (ZSTD_isError(rc)) {
            error = std::string("ZSTD_compressStream2 failed: ") + ZSTD_getErrorName(rc);
            return EncodeResult::Error;
        }
        ...
        if (rc == 0) {
            ...
            if (finished_) {
                if (!input_buffer_.empty()) {
                    error = "ZSTD_compressStream2 finished early";
                    return EncodeResult::Error;
                }
                return EncodeResult::Done;
            }
